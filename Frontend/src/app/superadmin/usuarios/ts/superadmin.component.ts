...
export class SuperAdminComponent implements OnInit {
    ...
    this.superAdminService.changeUserStatus(usuario.idUsuario, newStatus).subscribe({
      next: (response) => {
        ...
        Swal.fire({
          position: 'top-end',
          icon: 'success',
          title: `Estado del usuario ${usuario.nombreCompleto} actualizado correctamente`,
          showConfirmButton: false,
          timer: 2000,
        });
      },
  ...
  deleteUser(usuario: Usuario) {
    ...
    Swal.fire({
      title: `¿Está seguro que desea eliminar al usuario ${usuario.nombreCompleto}?`,
      text: 'Puede contener hoteles y habitaciones esta acción no se puede deshacer.',
      icon: 'warning',
        ...
        this.superAdminService.deleteUser(usuario.idUsuario!).subscribe({
          next: (response) => {
            ...
            Swal.fire({
              position: 'top-end',
              icon: 'success',
              title: `Usuario ${usuario.nombreCompleto} eliminado con éxito`,
              showConfirmButton: false,
              timer: 2000,
            });
          },
  ...
  confirmRoleChange() {
    if (!this.selectedUser || !this.selectedRole) {
      Swal.fire({
        position: 'top-end',
        icon: 'error',
        title: 'Debe seleccionar un usuario y un rol',
        showConfirmButton: false,
        timer: 3000,
      });
      return;

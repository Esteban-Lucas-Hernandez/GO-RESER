...
export class DetalleHabitacionComponent implements OnInit, AfterViewInit {
  ...
  enviandoReserva = false; // Flag para prevenir múltiples envíos
  ...
  loadHabitacionDetalle(hotelId: number, habitacionId: number): void {
    ...
    this.hotelService.getHabitacionDetalle(hotelId, habitacionId).subscribe({
      next: (data: HabitacionDetalle) => {
        ...
        // Inicializar la información del hotel con los datos que vienen en la habitación
        this.hotel = {
          id: hotelId,
          nombre: data.hotelNombre,
          descripcion: data.descripcionHotel,
        };
        
        // Cargar la información completa del hotel
  ...
  // Método para cargar la información completa del hotel
  loadHotelInfo(hotelId: number): void {
    this.hotelService.getHotelById(hotelId).subscribe({
      ...
      error: (err: any) => {
        ...
        this.hotel = {
          id: hotelId,
          nombre: this.habitacion.hotelNombre,
          descripcion: this.habitacion.descripcionHotel,
        };
      },
    });
  }
  ...
  onSubmitReserva(event: Event): void {
    ...
    const fechaInicio = this.reserva.fechaInicio;
    const fechaFin = this.reserva.fechaFin;
    ...
    // Validación 4: Prevenir múltiples envíos simultáneos
    if (this.enviandoReserva) {
      this.mostrarMensaje('Por favor espere mientras se procesa su reserva', 'error');
      return;
    }

    // Si pasa todas las validaciones, enviar la reserva
    this.enviarReserva();
  }

  private obtenerFechaHoraColombia(): string {
    const now = new Date();
    // Offset hora de Colombia -5 (sin horario de verano)
    const colombiaOffsetMinutos = -5 * 60;

    // UTC en ms
    const utc = now.getTime() + now.getTimezoneOffset() * 60000;
    const colombia = new Date(utc + colombiaOffsetMinutos * 60000);

    const pad = (n: number) => n.toString().padStart(2, '0');
    const yyyy = colombia.getFullYear();
    const mm = pad(colombia.getMonth() + 1);
    const dd = pad(colombia.getDate());
    const hh = pad(colombia.getHours());
    const min = pad(colombia.getMinutes());
    const ss = pad(colombia.getSeconds());
  ...
  enviarReserva(): void {
    if (!this.habitacionId || !this.reserva.fechaInicio || !this.reserva.fechaFin) return;

    // Marcar que estamos enviando para prevenir múltiples envíos
    this.enviandoReserva = true;

    // Enviar solo las fechas en formato YYYY-MM-DD (LocalDate) y tiempo Colombia en ISO
    const reservaRequest: ReservaRequest = {
      fechaInicio: this.reserva.fechaInicio.toISOString().split('T')[0],
      fechaFin: this.reserva.fechaFin.toISOString().split('T')[0],
      metodoPago: this.reserva.metodoPago,
      fechaHoraReserva: this.obtenerFechaHoraColombia(),
    };
    ...
    this.hotelService.crearReserva(this.habitacionId, reservaRequest).subscribe({
      next: (response: Reserva) => {
        ...
        this.pagoConfirmacion = {
          ...
          fechaPago: this.obtenerFechaHoraColombia(),
          monto: montoTotal,
        };

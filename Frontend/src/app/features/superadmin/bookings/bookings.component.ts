...
import { SuperAdminReserva, HotelSimple } from './bookings.service';
import { SuperAdminReservasService } from './bookings.service';
      ...
      this.reservasService.getReservasPorHotel(this.selectedHotelId).subscribe({
        ...
        error: (err: any) => {
          Swal.fire({
            position: 'top-end',
            icon: 'error',
            title: 'Error al cargar la lista de reservas',
            showConfirmButton: false,
            timer: 3000,
          });
  ...
  descargarPdf(idReserva: number): void {
    ...
    if (!idReserva || idReserva <= 0) {
      Swal.fire({
        position: 'top-end',
        icon: 'error',
        title: 'No se puede descargar el PDF: ID de reserva inválido.',
        showConfirmButton: false,
        timer: 3000,
      });
      return;
      ...
      error: (error: any) => {
        if (error.status === 403) {
          Swal.fire({
            position: 'top-end',
            icon: 'error',
            title: 'No tiene permisos para descargar el PDF de esta reserva.',
            showConfirmButton: false,
            timer: 3000,
          });
          ...
          Swal.fire({
            position: 'top-end',
            icon: 'error',
            title: 'No se encontró el PDF para esta reserva.',
            showConfirmButton: false,
            timer: 3000,
          });
        } else {
          Swal.fire({
            position: 'top-end',
            icon: 'error',
            title: 'No se pudo descargar el PDF. Por favor, inténtelo más tarde.',
            showConfirmButton: false,
            timer: 3000,
          });
        }
